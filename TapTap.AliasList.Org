...
#        <Activate> <Run> <Script> <Folder> <Site> <Etc>    # Alias 타입
#        # Alias 타입 설명
#        <Activate> Target, [, Working_Dir, Base_Option], Window_Title  # 타겟 프로그램 기동(실행) or 활성화
#        <Run> Target [, Working_Dir, Base_Option]    # 타겟 프로그램 실행(기동)
#        <Script> Alias   # Python Script 명령어
#        <Folder> Folder    # 폴더 열기
#        <Site> Web Site    # Web Site, Home Page 열기.
#        <Etc>  # 적당한 Alias Type이 없는 경우 지정 -> 우선 [Option] 에 대해 폴더 열기로 동작.
#        [대문자 혹은 숫자] 1 글자 # ARB 실행 시, Hotkey로 사용.
#        ? # 도움말 화면 보이기
#
...
<Alias> 부팅 초기 자동 실행 루틴
<OnBoot> c:\Program Files\AutoHotkey\AutoHotkey.exe z:\Workspace\TapTap\Lib\AHK\TapTap_Boot.ahk, Boot_Option

# 주석 란: 어떤 내용을 적어도 무시되는 줄(Line)
# 탭탭이에서 사용하는 명령어(Alias)를 명령어 종류 별로 구분하여 보여 줌.
<Alias> ? 탭탭이 별칭(별명, Alias) 목록
<Hotkey> z:\Workspace\TapTap\Lib\AHK\Hotkey_Help.ahk

# 숫자 Hotkey 테스트
# 숫자 1을 누르면 탭탭이가 인사함.
<Alias> 1, 인사하기
<Hotkey> c:\Program Files\AutoHotkey\AutoHotkey.exe /CP65001, z:\Workspace\TapTap\Lib\AHK\Hotkey_1.ahk
